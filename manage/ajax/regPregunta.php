...
$con=$cn->conectar();
$des=$_POST['dds'];
$categorias=$_POST['cates'];
$tema=$_POST['tema'];
$query1=mysqli_query($con,"select trae_id_pregunta() as ID ")or die(mysqli_error($con));
...
$resultado=array();
 ...
 for($i=0;$i<count($categorias);$i++){
   $idct=$categorias[$i];
   $query3=mysqli_query($con,"insert into CategoriaPregunta values('$idct','$idp')")or die(mysqli_error($con));
 }
 
 $query2=mysqli_query($con,"insert into pregunta values('$idp','$tema','$des')")or die(mysqli_error($con));
if($query2 && $query3){
